        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            ...
            {!isLoading && suppliers.map(s => (
              <tr key={s.id} className="hover:bg-muted/30 transition-colors group">
                <td className="px-4 py-3">
                  ...
                  <div className="text-xs text-muted-foreground">{s.company_name || s.document || "Sem dados fiscais"}</div>
                </td>
                <td className="px-4 py-3 text-muted-foreground">
                  <div>{s.email || "Sem email"}</div>
                  <div className="text-xs">{s.phone || "Sem telefone"}</div>
                </td>
            ...
            {!isLoading && suppliers.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-12 text-center text-sm text-muted-foreground">Nenhum fornecedor encontrado.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function SupplierFormDrawer({ open, onOpenChange, supplier }: { open: boolean, onOpenChange: (open: boolean) => void, supplier: Partial<Supplier> | null }) {
  ...
  useEffect(() => {
    if (open && supplier) {
      setFormData({
        ...supplier,
        cnpj: (supplier as any).document || (supplier as any).cnpj || "",
        whatsapp: (supplier as any).phone || (supplier as any).whatsapp || ""
      });
    }
  }, [open, supplier]);
