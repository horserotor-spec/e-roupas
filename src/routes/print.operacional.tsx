...
function PrintOperacionalPage() {
  ...
  // Definir o título da página com o código do pedido
  // O navegador usa o <title> como nome sugerido ao salvar em PDF
  useEffect(() => {
    const prev = document.title;
    if (order?.code) {
      document.title = `Etiquetas - ${order.code}`;
    }
        ...
        {finalSheetLabels.map((lbl, idx) => {
          ...
          return (
            <div key={idx} className="label-cell text-black">
              ...
              {/* LINHA 1: Pedido | Peça X/Y | Tamanho */}
