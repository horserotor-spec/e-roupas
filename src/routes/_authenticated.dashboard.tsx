...
import { orders, clients, statusLabel, statusTone, isOverdue, clientById } from "@/lib/mock-data";
import { StatusBadge } from "@/components/StatusBadge";
import { ArrowUpRight, Flame, Clock, FileCheck2, UserPlus, Activity } from "lucide-react";
...
function Dashboard() {
  const today = orders.filter((o) => o.createdAt >= "2026-05-28");
  const urgent = orders.filter((o) => o.urgent);
  const waitingArt = orders.filter((o) => o.status === "aguardando_arte");
  const overdue = orders.filter(isOverdue);
  const newClients = clients.filter((c) => c.createdAt >= "2025-04-01");

  return (
    <div className="px-6 md:px-10 py-8 max-w-[1400px] mx-auto">
      <div className="flex items-end justify-between mb-8">
        <div>
          <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Sábado, 30 de maio</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <KpiCard to="/pedidos" label="Urgentes" value={urgent.length} hint="Furam fila" icon={Flame} accent="warning" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mt-3">
        <section className="lg:col-span-2 rounded-2xl border border-border bg-card p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Atalho para o núcleo do ERP.</p>
            </div>
          ...
          <div className="divide-y divide-border -mx-2">
            {orders.slice(0, 5).map((o) => {
              ...
              return (
                ...
                >
                  <div className="text-xs font-mono text-muted-foreground number w-32 shrink-0">{o.code}</div>
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-medium truncate">{c?.name}</div>
                    <div className="text-xs text-muted-foreground truncate">{o.items.map((i) => `${i.qty}× ${i.product}`).join(" · ")}</div>
                  </div>
                  <StatusBadge tone={statusTone[o.status]}>{statusLabel[o.status]}</StatusBadge>
                  <div className="text-sm font-medium number w-24 text-right">R$ {o.total.toLocaleString("pt-BR")}</div>
                </Link>
              );
            })}
          </div>
        ...
        <section className="rounded-2xl border border-border bg-card p-5">
          <h2 className="text-sm font-semibold tracking-tight mb-1">Atividades recentes</h2>
          <p className="text-xs text-muted-foreground mb-4">Da timeline universal.</p>
          <ul className="space-y-3">
            {orders.flatMap((o) => o.timeline.map((t) => ({ o, t })))
              .sort((a, b) => b.t.at.localeCompare(a.t.at))
              .slice(0, 6)
              .map(({ o, t }) => (
                <li key={t.id} className="text-xs flex gap-3">
                  <span className="mt-1 size-1.5 rounded-full bg-primary shrink-0" />
                  <div className="min-w-0">
                    <Link to="/pedidos/$orderId" params={{ orderId: o.id }} className="font-medium hover:underline">{o.code}</Link>
